...
int main()
{
	...
	int new_mark_flag[place_num];//当前标识的标志位 
	struct Node *q;
	q=head_Node;
	do
	{
...
void Initialize()
{
	...
	for(i=0;i<place_num;i++)//初始化第一个结点的具体标识的标志位 
	{
...
void product_new_m(int new_m[],int new_m_flag[])
{
	...
	for(Transition=0;Transition<transition_num;Transition++)//循环判断可发生的变迁 
	{
		...
		if(can_transition==1)
		{
			add_to_node(new_m,new_m_flag);//进入公示计算和其他标志位赋值 
		}
	}
}
...
//	
	...
	do
	{
		for(i=0;i<place_num;i++)
		{
			if(former->new_m[i]==P->new_m[i])//生成的新标识与链表的第一个结点的标识每一位判断是否相同（相同为真） 
			{
				same=1;
			}
			else
...
void out_put(void)
{
	...
	do
	{
		...
		{
			printf("%s"," 它不是覆盖标识");
			fprintf(fp,"%s"," 它不是覆盖标识");
		}
		else
		{
			printf("%s"," 它的覆盖标识是");
			fprintf(fp,"%s"," 它的覆盖标识是");
